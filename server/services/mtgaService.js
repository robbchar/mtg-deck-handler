...
/**
 * Regex that captures the MTGA Arena set/collector suffix appended to card names.
 *
 * Format: " (SET) collectorToken"
 *
 * Capturing groups:
 *   [1] set code        — e.g. "FDN", "ANB", "PRM"
 *   [2] collector token — e.g. "279", "2017F", "279a", "★"
 *
 * Intentionally permissive to handle all MTGA export variants:
 ...
 *
 * @param {string} text
 * @returns {{ mainboard: Array<{quantity: number, name: string, set_code?: string, collector_number?: string}>,
...
function parseMtgaText(text) {
  ...
  for (const rawLine of lines) {
    ...
    const name = suffixMatch
      ...
      : rawName;
    const set_code = suffixMatch ? suffixMatch[1] : undefined;
    const collector_number = suffixMatch ? suffixMatch[2] : undefined;
